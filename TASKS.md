...
### Task 12-1: パスワード設定機能
- [ ] Google認証ユーザーが後からパスワードを設定できる機能
- [ ] 設定画面に「パスワード設定」セクション追加
- [ ] Supabase `updateUser({ password })` で実装
- [ ] Google障害時のバックアップログイン手段として

### Task 12-2: 認証バグ修正（完了）
- [x] 登録時の409 Conflictエラー修正
- [x] RLS 406エラー修正（API Route + Service Role使用）
- [x] handle_new_userトリガー作成
